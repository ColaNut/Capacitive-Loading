...
thickness_da    = 1 / 100; % 20 cm (down air)
tmp_thickness   = [ 2.0, 0.02, 3.5, 1.00, 2.8, 15, 3, 0.2, 2.0 ]' ./ 100; % cm -> m, total = 31 cm
thickness       = flipud(tmp_thickness);
thickness_10    = 5 / 100; % 15 cm
% thickness     = [ 2, 2, 4, 2, 3, 15, 3, 2, 2 ]' ./ 100; % cm -> m, total = 31 cm
tmp_epsilon_r   = [ 113.0, 303.1, 184, 264.9, 402, 264.9, 184, 303.1, 113.0 ]';
epsilon_r       = flipud(tmp_epsilon_r);
epsilon_b_r     = epsilon_r(1);
epsilon_0_r     = 1;
epsilon         = epsilon_r * Epsilon_0;
epsilon_b       = epsilon(1);
tmp_sigma       = [ 0.61, 0.33, 0.685, 0.42, 0.68, 0.42, 0.685, 0.33, 0.61 ]';
sigma           = flipud(tmp_sigma);
...
d_z_da          = 1 ./ 100;
tmp_d_z         = [ 0.5, 0.01, 0.5, 0.5, 1.4, 5, 1, 0.1, 0.5 ]' ./ 100; % 0.1cm
d_z             = flipud(tmp_d_z);
d_z_10          = 1 ./ 100; % 5cm
...
A_da( M_x_max * M_y_max * N_da + 1: M_x_max * M_y_max * ( N_da + N_max + N_10 )...
        , M_x_max * M_y_max * N_da + 1: M_x_max * M_y_max * ( N_da + N_max + N_10 ) ) ...
    = A_10( 1: M_x_max * M_y_max * ( N_max + N_10 ), : );
...
%     if length( find( A_da( :, idx ) ) ) >= 8
%         idx
%     end
% end
for idx = 1: 1: M_x_max * M_y_max * ( N_da + N_max + N_10 ) 
    tmp_max = max( abs(A_da( idx, : )) );
    A_da( idx, : ) = A_da( idx, : ) ./ max( abs(A_da( idx, : )) );
    B_da( idx ) = B_da( idx ) ./ tmp_max; 
    % % idx = ( ell - 1 ) * M_x_max * M_y_max + ( n - 1 ) * M_x_max + m;
    % tmp_m = mod( idx, M_x_max );
    % if tmp_m == 0
    %     m = M_x_max;
