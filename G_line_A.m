...
switch line_idx
    case 1
        ...
        A_row(6)  = p6;
        A_row(7)  = p0;
        A_row(8)  = ( epsilon(p + 1) + epsilon_b ) * d_x * d_y / ( 2 * d_z(p + 1) ); % ell + 1
        A_row(9)  = epsilon(p + 1) * d_y * d_z(p + 1) / (2 * d_x) + epsilon(p) * d_y * d_z(p) / (2 * d_x); % m - 1
        A_row(10) = ( epsilon(p) + epsilon_b ) * d_x * d_y / ( 2 * d_z(p) ); % ell - 1
        A_row(11) = epsilon_b * d_y * ( d_z(p) + d_z(p + 1) ) / (2 * d_x); % m + 1
        A_row(12) = epsilon(p + 1) * d_x * d_z(p + 1) / (4 * d_y) + ...
    ...
    case 2
        ...
        A_row(10) = ( epsilon(p) + epsilon_b ) * d_x * d_y / ( 2 * d_z(p) ); % ell - 1
        A_row(11) = epsilon(p + 1) * d_y * d_z(p + 1) / (2 * d_x) + epsilon(p) * d_y * d_z(p) / (2 * d_x); % m + 1
        A_row(12) = epsilon(p + 1) * d_x * d_z(p + 1) / (4 * d_y) ...
            + epsilon(p) * d_x * d_z(p) / (4 * d_y) + epsilon_b * d_x * ( d_z (p) + d_z(p + 1) ) / (4 * d_y);  % n + 1
        A_row(13) = A_row( 12 ); % n - 1
        A_row(14) = - ( A_row( 8 ) + A_row( 9 ) + A_row( 10 ) + A_row( 11 ) + A_row( 12 ) + A_row( 13 ) ); % center point
    case 3
        ...
        A_row(7)  = p0;
        A_row(8)  = ( epsilon(p + 1) + epsilon_b ) * d_x * d_y / ( 2 * d_z(p + 1) ); % ell + 1
        A_row(9)  = epsilon(p + 1) * d_y * d_z(p + 1) / (4 * d_x) ...
        ...
        A_row(13) = epsilon(p + 1) * d_x * d_z(p + 1) / (2 * d_y) + epsilon(p) * d_x * d_z(p) / (2 * d_y); % n - 1
        A_row(14) = - ( A_row( 8 ) + A_row( 9 ) + A_row( 10 ) + A_row( 11 ) + A_row( 12 ) + A_row( 13 ) ); % center point
    case 4
        A_row(1)  = p1;
        A_row(2)  = p2;
