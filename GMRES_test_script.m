...
% % Check if the two B_10 are the same
...
% % ext_itr_num = 20;
% x_zero = B_10;
% % Test for another x_zero: linearly decreasing from top plate to the bottom plate.
...
% save('GMRES_test_script_sparse2.mat');
% tmp_GMRES = ratio_my_gmres;

% % load('GMRES_test_script.mat');
...
% ratio_diff = norm(ratio_my_gmres - tmp_GMRES) / norm(ratio_my_gmres)

% % save('GMRES_test_script_sparse.mat');
...
legend_handle = legend( '${\rm Itr}_{int}$ = 5', '${\rm Itr}_{int}$ = 10', '${\rm Itr}_{int}$ = 15', '${\rm Itr}_{int}$ = 20', '${\rm Itr}_{int}$ = 25', '${\rm Itr}_{int}$ = 30', '${\rm Itr}_{int}$ = 35', '${\rm Itr}_{int}$ = 40', 'Location', 'eastoutside' );
set(legend_handle,'Interpreter','latex');

% bar_x_my_gmres = GMRES_test( A_10_sparse, B_10, 25, 15, tol, x_zero );
% Errpr_15 = 100 * norm( bar_x_10 - bar_x_my_gmres ) / norm(bar_x_10);
% figure(2);
% cross_section_test_gmres;

% bar_x_my_gmres = GMRES_test( A_10_sparse, B_10, 25, 20, tol, x_zero );
% Errpr_20 = 100 * norm( bar_x_10 - bar_x_my_gmres ) / norm(bar_x_10);
% figure(3);
% cross_section_test_gmres;

% bar_x_my_gmres = GMRES_test( A_10_sparse, B_10, 25, 35, tol, x_zero );
